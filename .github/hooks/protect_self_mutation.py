...
CORE_IMPORT_RESEARCH_PATTERN = re.compile(
    r"^\s*(?:from|import)\s+jaya_research\b|\bjaya_research\.",
    re.IGNORECASE,
)
RESEARCH_IMPORT_CORE_PATTERN = re.compile(
    r"^\s*(?:from|import)\s+jaya_core\b|\bjaya_core\.",
    re.IGNORECASE,
)


LOOP_PATTERNS = [
    re.compile(r"(?:^|\s)(?:python(?:3)?\s+)?(?:packages[\\/]jaya-research[\\/]src[\\/]jaya_research[\\/])?discovery(?:[\\/]__init__)?\.py\b[^\n\r]*\s--forever\b", re.IGNORECASE),
    re.compile(r"(?:^|\s)(?:python(?:3)?\s+)?(?:packages[\\/]jaya-research[\\/]src[\\/]jaya_research[\\/])?jit_discovery\.py\b[^\n\r]*\s--forever\b", re.IGNORECASE),
]
        ...
        if "/" not in rel_path:
            if lower not in ROOT_FILE_ALLOWLIST:
                return (
                    ...
                    "Place files inside packages/, tests/, docs/, or .github/."
                )
...
def _check_cross_scope_imports(python_updates: list[tuple[str, str]]) -> str | None:
    for rel_path, line in python_updates:
        ...
        if lower_path.startswith("packages/jaya-core/") and CORE_IMPORT_RESEARCH_PATTERN.search(line):
            return (
                f"Cross-domain import blocked in {rel_path}: "
                "jaya_core must not import jaya_research directly."
            )
        if lower_path.startswith("packages/jaya-research/") and RESEARCH_IMPORT_CORE_PATTERN.search(line):
            return (
                f"Cross-domain import blocked in {rel_path}: "
