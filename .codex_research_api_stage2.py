...
def _run_research_job(job_id: str) -> None:
    ...
    try:
        ...
        agent_type = _load_optional_dependency(`r`n            "ResearchAgent", "research.agent", "ResearchAgent"`r`n        )`r`n        agent = agent_type(
            topic=topic,
            focus_areas=focus,
...
async def get_experiment_chart(exp_id: str, metric: str):
    ...
    tracker_type = _load_optional_dependency(`r`n        "ExperimentTracker", "research.academic.tracker", "ExperimentTracker"`r`n    )`r`n    tracker = tracker_type()
...
async def chat_with_knowledge(request: ChatRequest):
    ...
    if any(t in msg_lower for t in triggers):
        try:
            ...
            processor_type = _load_optional_dependency(`r`n                "JournalProcessor",`r`n                "research.academic.journal_processor",`r`n                "JournalProcessor",`r`n            )`r`n            processor = processor_type()
...
def get_history():
    ...
    if meta_analyst is None:
        analyst_type = _load_optional_dependency(`r`n            "MetaAnalyst", "research.meta_analysis", "MetaAnalyst"`r`n        )`r`n        meta_analyst = analyst_type()
