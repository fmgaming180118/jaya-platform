...
Verifies TextInterface CLI commands, VoiceInterface transcript processing, and FastAPI endpoint routes.
"""
...
class TestPhase3Interfaces(unittest.TestCase):
    ...
    def setUp(self):
        self.text_if = TextInterface(agent_loop=AgentLoop(enable_teacher_fallback=False))
        self.voice_if = VoiceInterface(agent_loop=AgentLoop(enable_teacher_fallback=False))
    ...
    def test_cli_slash_commands(self):
        ...
        self.assertIn("207 verified patches", patches_resp)
    ...
    def test_api_server_endpoints(self):
        ...
        chat_res = self.client.post("/agent/chat", json={"prompt": "Explain Phase 3 Interface"})
