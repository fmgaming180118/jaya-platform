...
from runtime.perception import PerceptionPipeline, EventType
...
class TestPhase1AgentLoop(unittest.TestCase):
    ...
    def test_perception_intent_classification(self):
        ...
        event_coding = self.perception.process("Write a Python script to optimize memory")
        self.assertEqual(event_coding.intent, "coding_task")
        self.assertGreaterEqual(event_coding.confidence, 0.90)

        event_search = self.perception.process("Search latest quantum computing research on duckduckgo")
