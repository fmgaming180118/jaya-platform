...
def test_manifest_matches_jaya_flags():
    ...
    assert (
        len(flag_names_in_schema) == 40
    ), f"Expected 40 core flags, found {len(flag_names_in_schema)}"

    manifest_flag_names = {p["source_flag"].replace("JayaFlags.", "") for p in manifest}

    assert (
        manifest_flag_names == flag_names_in_schema
    ), "Manifest flags do not perfectly match JayaFlags schema"
...
def test_canonical_names_snapshot():
    ...
    ), "Canonical names do not match the expected snapshot!"
...
def test_binary_flag_values_snapshot():
    ...
    for flag_name, expected_val in expected_values.items():
        ...
        ), f"Binary compatibility broken! {flag_name} expected {expected_val}, got {actual_val}"


def test_40_pillar_documentation_has_canonical_entrypoints():
    docs_index = (PROJECT_ROOT / "docs" / "README.md").read_text(encoding="utf-8")
    pillar_index = (PROJECT_ROOT / "docs" / "pillars" / "README.md").read_text(
        encoding="utf-8"
    )

    assert "arsitektur_40_pilar_jaya.md" in docs_index
    assert "40_PILLARS_IMPLEMENTATION_MATRIX.md" in docs_index
    assert "pillars/README.md" in docs_index
    assert "../40_PILLARS_IMPLEMENTATION_MATRIX.md" in pillar_index
