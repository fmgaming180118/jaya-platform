...
from src.memory.episodic import EpisodicMemoryStore, MemoryEvent
from src.memory.working import WorkingMemory
from src.cognitive.context import ContextManager
from src.resources.profiler import ResourceProfile
from src.resources.modes import ExecutionMode
from src.identity.models import NodeIdentity, NodeClass
...
class TestMultiTurnConversationMemoryRestart(unittest.TestCase):
    ...
    def test_narrative_continuity_full_restart_cycle(self):
        ...
        for i, turn in enumerate(turns):
            ...
            # In stub mode, execute_intent returns ok=False but still records narrative
            # The important thing is narrative continuity works
        ...
        narrative1 = engine1.narrative_context(limit=20, max_chars=2000)
        ...
        narrative2 = engine2.narrative_context(limit=20, max_chars=2000)
        ...
        narrative3 = engine2.narrative_context(limit=20, max_chars=2000)
    ...
    def test_context_manager_integration_restart(self):
        ...
        context1 = engine1.narrative_context(limit=10, max_chars=1000)
        
        del engine1
        
        # Restart
        engine2 = self._build_engine()
        context2 = engine2.narrative_context(limit=10, max_chars=1000)
        
        # Context should be restored
        self.assertIn("Python", context2["context"])
        ...
        from src.identity.models import NodeRole, AuthorityLevel
    ...
    def test_memory_bounds_respected_after_restart(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"JAYA_NARRATIVE_PATH": narrative_path}, clear=False):
                engine1 = IronEngine(model_path="missing.jay", password="x", enable_twin=False)
                engine1.ignite()
                ...
                # Restart
                engine2 = IronEngine(model_path="missing.jay", password="x", enable_twin=False)
                engine2.ignite()
                
                narrative2 = engine2.narrative_context(limit=100, max_chars=5000)
                self.assertLessEqual(narrative2["total_events"], 256)
                self.assertLessEqual(len(narrative2["context"]), 2000)  # Bounded by max_chars in snapshot
