...
import pytest
from src.rag.research_artifact import ResearchArtifactVerificationError
from src.rag.research_bridge import ResearchBridge, ResearchInboxError
...
def test_simulation_and_missing_confidence_never_enter_core(tmp_path: Path) -> None:
    ...
    with pytest.raises(ResearchArtifactVerificationError, match="confidence"):
        from src.rag.research_artifact import verify_research_artifact
...
def test_legacy_promoted_discovery_is_quarantined() -> None:
    ...
    discovery_path = core_root / "src/brain_v2/engine/research/discovery.py"
    ...
    from src.brain_v2.engine.research.discovery import (
