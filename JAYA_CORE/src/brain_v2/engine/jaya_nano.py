...
class JayaNanoEngine:
    ...
    def _find_native_library(self) -> Optional[str]:
        ...
        possible_paths = [
            ...
            str(Path(__file__).parent.parent.parent / "JAYA_ANDROID" / "app" / "build" / "intermediates" / "merged_native_libs" / "debug" / "out" / "lib" / "arm64-v8a" / "libllama.so"),
            # Development paths
            str(Path(__file__).parent.parent.parent / "llama.cpp" / "build" / "bin" / "libllama.so"),
            str(Path(__file__).parent.parent.parent / "llama.cpp" / "build" / "bin" / "Release" / "libllama.so"),
            # System paths
