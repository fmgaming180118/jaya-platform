...
Usage:
    C:/Users/Lenovo/AppData/Local/Programs/Python/Python312/python.exe \
        JAYA_CORE/scripts/benchmark_phase1_ir.py
"""
...
def evaluate_gate(results: Sequence[BenchmarkResult], gate: GateConfig) -> GateStatus:
    ...
    for r in results:
        if r.warm_p50_ms > gate.max_warm_p50_ms:
            failures.append(
                f"{r.label}: warm_p50_ms={r.warm_p50_ms:.4f} > {gate.max_warm_p50_ms:.4f}"
            )
        if r.warm_p95_ms > gate.max_warm_p95_ms:
            failures.append(
                f"{r.label}: warm_p95_ms={r.warm_p95_ms:.4f} > {gate.max_warm_p95_ms:.4f}"
            )
        ...
        if r.warm_p50_ms > r.cold_p50_ms:
            failures.append(
                f"{r.label}: warm_p50_ms={r.warm_p50_ms:.4f} worse than cold_p50_ms={r.cold_p50_ms:.4f}"
            )
        if r.warm_p95_ms > r.cold_p95_ms:
            failures.append(
                f"{r.label}: warm_p95_ms={r.warm_p95_ms:.4f} worse than cold_p95_ms={r.cold_p95_ms:.4f}"
            )
...
def run_policy_grid(policies: Sequence[Tuple[int, float]], rounds: int) -> List[BenchmarkResult]:
...
if __name__ == "__main__":
    ...
    policies = tuple(_parse_policy(raw) for raw in args.policy) if args.policy else DEFAULT_POLICIES
