...
_AGENT_PACKAGE = _AGENT_SOURCE / "jaya_agent"
if str(_AGENT_SOURCE) not in sys.path:
    sys.path.insert(0, str(_AGENT_SOURCE))

from jaya_agent.interfaces.text_interface import TextInterface
...
def test_agent_skills_do_not_import_research_or_mutate_sys_path() -> None:
    ...
    interface_source = (_AGENT_PACKAGE / "interfaces/text_interface.py").read_text(
        encoding="utf-8"
    )
