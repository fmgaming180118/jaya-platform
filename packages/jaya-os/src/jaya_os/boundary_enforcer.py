...
from `jaya_os.*`.  The only approved import path is:
    packages/jaya-agent/src/jaya_agent/security/capability_sandbox.py

Any other file in packages/jaya-core or packages/jaya-agent that imports directly from
jaya_os.* is a boundary violation and will be reported.

Usage:
    python packages/jaya-os/src/jaya_os/boundary_enforcer.py
    python packages/jaya-os/src/jaya_os/boundary_enforcer.py --repo-root /path/to/repo
"""
...
_APPROVED_ADAPTER = (
    ...
    / "jaya_agent"
    / "security"
    / "capability_sandbox.py"
)

# Directories scanned for violations (relative to repo root)
_SCAN_DIRS = ["packages/jaya-core", "packages/jaya-agent"]
...
class OsBoundaryEnforcer:
    ...
    Only the Agent capability-sandbox adapter is allowed to import directly
    from `jaya_os.*`. All other Python files in Core and Agent must go through
    that adapter.
    """
