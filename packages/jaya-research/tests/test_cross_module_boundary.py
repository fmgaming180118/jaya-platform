...
import pytest
from jaya_research.brain.system_control import SystemController
from jaya_research.capability_boundary import (
    ArbitraryExecutionRejected,
    CapabilityUnavailableError,
)
from jaya_research.memory_manager import EcosystemBoundaryError, MemoryManager
from jaya_research.sandbox import Sandbox
from jaya_research.tools.system.cmd_runner import CmdRunnerTool
from jaya_research.voice_agent.agent import JayaVoiceAgent

_RESEARCH_ROOT = Path(__file__).resolve().parents[1]
_WORKSPACE_ROOT = _RESEARCH_ROOT.parents[1]
...
def test_scoped_legacy_files_contain_no_process_or_cross_copy_primitive() -> None:
    relative_paths = (
        "src/jaya_research/brain/system_control.py",
        "src/jaya_research/tools/system/cmd_runner.py",
        "src/jaya_research/sandbox/__init__.py",
        "tests/legacy_source/test_execution.py",
        "src/jaya_research/memory_manager.py",
        "src/jaya_research/discovery/__init__.py",
        "src/jaya_research/refactor.py",
        "src/jaya_research/launcher.py",
        "src/jaya_research/voice_agent/agent.py",
...
def test_root_legacy_scripts_are_verifier_or_canonical_wrapper_only() -> None:
    ...
    assert "from jaya_core.core_service import create_app" in server


def test_discovery_exports_candidate_without_mutating_source(tmp_path: Path) -> None:
    from jaya_research.discovery import ScientificDiscovery
...
def test_discovery_refactor_and_launcher_have_no_legacy_mutation_loop() -> None:
    scoped_forbidden = {
        ...
        "src/jaya_research/refactor.py": ("write_text(", "D:\\", "os.walk("),
        "src/jaya_research/launcher.py": ("subprocess", "os.system", "shell=True"),
    }
