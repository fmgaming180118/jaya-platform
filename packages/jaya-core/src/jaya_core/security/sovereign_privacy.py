...
from jaya_core.privacy_redaction import redact_private
...
_CLASSIFICATION_CODES = {value: index for index, value in enumerate(DataClassification)}
_PURPOSE_CODES = {value: index for index, value in enumerate(DataPurpose)}
_DESTINATION_CODES = {value: index for index, value in enumerate(DataDestination)}
...
class PrivacyFailureCode(str, Enum):
    ...
    CONSENT_TRUST_UNAVAILABLE = "PRIVACY_CONSENT_TRUST_UNAVAILABLE"
...
class SovereignPrivacy:
    ...
    def __init__(
        ...
        *,
        ...
        consent_key_resolver: Callable[[str], bytes | str | None] | None = None,
        clock: Callable[[], datetime] = _now,
        privacy_gate: TrustedPrivacyGate | None = None,
    ) -> None:
        if consent_public_keys and consent_key_resolver is not None:
            raise PrivacyError(
                PrivacyFailureCode.INVALID_INPUT,
                "static consent keys and dynamic resolver are mutually exclusive",
            )
        self._clock = clock
        try:
            self._privacy_gate = privacy_gate or get_trusted_privacy_gate()
        except NativeProviderError as exc:
            raise PrivacyError(
                PrivacyFailureCode.NOT_CONFIGURED,
                "trusted privacy scope provider is unavailable",
            ) from exc
        self._lock = threading.RLock()
        self._connection = sqlite3.connect(str(db_path), timeout=5.0, check_same_thread=False)
        ...
        self._consent_key_resolver = consent_key_resolver
    ...
    def install_consent(self, grant: ConsentGrant) -> str:
        ...
        key = self._resolve_consent_key(grant.approver_id)
        if key is None:
            raise PrivacyError(
    ...
    def evaluate(self, request: PrivacyUseRequest) -> PrivacyDecision:
        ...
        contract = PrivacyUseContract(
            ...
            classification_code=_CLASSIFICATION_CODES[request.classification],
            purpose_code=_PURPOSE_CODES[request.purpose],
            destination_code=_DESTINATION_CODES[request.destination],
        )
        consent: ConsentGrant | None = None
        try:
            trusted_decision = self._privacy_gate.evaluate(contract)
            if trusted_decision.reason_code == "SIGNED_CONSENT_REQUIRED" and request.consent_id:
                consent = self._load_active_consent(request)
                if consent is not None:
                    trusted_decision = self._privacy_gate.evaluate(
                        contract,
                        VerifiedConsentScope(
                            owner_id=consent.owner_id,
                            subject_id=consent.subject_id,
                            classification_mask=sum(
                                {
                                    1 << _CLASSIFICATION_CODES[value]
                                    for value in consent.classifications
                                }
                            ),
                            purpose_mask=sum(
                                {1 << _PURPOSE_CODES[value] for value in consent.purposes}
                            ),
                            provider_ids=consent.providers,
                        ),
                    )
        except NativeProviderError as exc:
            raise PrivacyError(
                PrivacyFailureCode.NOT_CONFIGURED,
                "trusted privacy scope evaluation failed",
            ) from exc
        if trusted_decision.reason_code == "CONSENT_SCOPE_INVALID":
            raise PrivacyError(
                PrivacyFailureCode.CONSENT_INVALID,
                "consent scope does not match the data use request",
            )
        effect = PrivacyEffect.ALLOW if trusted_decision.effect == 1 else PrivacyEffect.DENY
        reason = trusted_decision.reason_code
        consent_id: str | None = None
        if reason == "SIGNED_CONSENT_ALLOWED" and consent is not None:
        ...
        try:
            with self._lock, self._connection:
                existing = self._connection.execute(
                    "SELECT descriptor_json, deleted_at FROM privacy_vault WHERE data_id = ?",
                    (data_id,),
                ).fetchone()
                if existing is not None:
                    ...
                    if (
                        ...
                        and previous.get("plaintext_sha256") == descriptor.plaintext_sha256
                        ...
                        return DataDescriptor(
                            ...
                            classification=DataClassification(previous["classification"]),
                            allowed_purposes=tuple(
                                DataPurpose(item) for item in previous["allowed_purposes"]
                            ),
    ...
    def export_owner(self, owner_id: str) -> dict[str, object]:
        rows = self._connection.execute(
            "SELECT data_id FROM privacy_vault WHERE deleted_at IS NULL ORDER BY data_id"
        ).fetchall()
        records: list[dict[str, object]] = []
        for row in rows:
            try:
                _, descriptor = self._load_vault_record(row["data_id"])
            except PrivacyError:
                continue
    ...
    def delete_owner(self, owner_id: str) -> dict[str, object]:
        ...
        with self._lock, self._connection:
            rows = self._connection.execute(
                "SELECT data_id, descriptor_json FROM privacy_vault WHERE deleted_at IS NULL"
            ).fetchall()
            targets = [
                row["data_id"]
                for row in rows
    ...
    def plaintext_absent(self, needle: str) -> bool:
        ...
        return all(needle not in "".join(str(value) for value in tuple(row)) for row in rows)
    ...
    def status(self) -> dict[str, object]:
        ...
        return {
            ...
            "consent_trust_source": (
                "dynamic_resolver" if self._consent_key_resolver is not None else "static_mapping"
            ),
            "privacy_scope_provider": self._privacy_gate.profile(),
    ...
    def _load_active_consent(self, request: PrivacyUseRequest) -> ConsentGrant | None:
        ...
        grant = ConsentGrant(
            ...
            classifications=tuple(DataClassification(item) for item in raw["classifications"]),
    ...
    def _resolve_consent_key(self, approver_id: str) -> Ed25519PublicKey | None:
        if self._consent_key_resolver is None:
            return self._consent_keys.get(approver_id)
        try:
            material = self._consent_key_resolver(approver_id)
            if material is None:
                return None
            return self._load_keys({approver_id: material})[approver_id]
        except PrivacyError:
            raise
        except Exception as exc:
            raise PrivacyError(
                PrivacyFailureCode.CONSENT_TRUST_UNAVAILABLE,
                "dynamic consent trust registry is unavailable",
            ) from exc
    ...
    def _load_vault_record(self, data_id: str) -> tuple[sqlite3.Row, DataDescriptor]:
        ...
        try:
            ...
            descriptor = DataDescriptor(
                ...
                allowed_purposes=tuple(DataPurpose(item) for item in raw["allowed_purposes"]),
    ...
    def _record_event(self, event: str, payload: Mapping[str, Any]) -> dict[str, object]:
