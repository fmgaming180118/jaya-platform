...
DEFAULT_PROCESS_PROFILES = {
    "pytest.workspace": ProcessProfile(
        ...
        allowed_args=[
            ...
            r"^--version$",
            r"^tests/.*\.py$",
            r"^packages/jaya-core/tests/.*\.py$",
            r"^packages/jaya-research/tests/.*\.py$",
            r"^packages/jaya-agent/tests/.*\.py$",
            r"^packages/jaya-os/tests/.*\.py$",
        ],
        max_args=5,
...
from jaya_core.security.approval_authority import ApprovalReceipt, ApprovalAuthority
...
class CognitiveAgentBridge:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Cross the OS boundary only through the Agent-owned adapter.
            from jaya_agent.security.capability_sandbox import (
                ActionPolicy,
                CapabilitySandbox,
            )
            from pathlib import Path
    ...
    def _register_process_profiles(self, workspace_root: Path) -> None:
        ...
        from jaya_agent.security.capability_sandbox import (
            ProcessProfile as OSProcessProfile,
        )
        import sys
    ...
    def execute_cognitive_plan(
        ...
        if not execution_state:
            from jaya_core.cognitive.execution_state import PlanExecutionState
