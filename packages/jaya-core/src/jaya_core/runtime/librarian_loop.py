...
from jaya_core.paths import core_models_dir

logger = logging.getLogger("LibrarianLoop")

class LibrarianLoop:
    def __init__(self, model_path: str | Path | None = None, allow_bootstrap_fallback: bool = False):
        resolved_model_path = (
            Path(model_path)
            if model_path is not None
            else core_models_dir() / "jaya-core-v0"
        )
        self.model = NativeJayaLibrarianRuntime(model_dir=str(resolved_model_path))
        ...
        try:
            ...
            if self.allow_bootstrap_fallback:
                ...
                from jaya_core.model.architecture import BootstrapLibrarianModel
                self.model = BootstrapLibrarianModel(
