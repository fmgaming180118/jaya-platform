...
def _ternary_linear(x: np.ndarray, W: np.ndarray) -> np.ndarray:
    ...
    """
    return get_compute_provider().ternary_linear(x, W)
...
class NanoModel:
        ...
        def verified_array(value: Any, shape: tuple[int, ...]) -> np.ndarray:
            ...
            try:
                get_trusted_artifact_gate().validate_ternary_values(
                    normalized, max_length=64 * 1024 * 1024
                )
            except NativeProviderError as exc:
                code = (
                    ModelFailureCode.DEPENDENCY_MISSING
                    if exc.code == "PROVIDER_UNAVAILABLE"
                    else ModelFailureCode.WEIGHTS_INVALID
                )
                raise ModelUnavailableError(code) from exc
            return normalized
    ...
    def get_config(self) -> dict[str, Any]:
        return {
            ...
            "version": "V18.0",
            "compute_provider": get_compute_provider().provider_id,
            "trusted_artifact_provider": get_trusted_artifact_gate().provider_id,
        }
