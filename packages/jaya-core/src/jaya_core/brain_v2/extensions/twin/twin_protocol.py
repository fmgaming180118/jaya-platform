...
- bounded in-memory state with ACID disk persistence across restarts.
"""
...
logger = logging.getLogger("TwinProtocol")
...
class TwinProtocol:
    """Signed twin handshake/sync protocol with replay, stale sequence guards, and persistence."""
    ...
    def save_state(self, path: Optional[str | Path] = None) -> bool:
        """Atomically persist current peer sequences, nonces, and counters."""
        ...
        state = {
            ...
            "tx_seq": self._tx_seq,
            "last_seq_by_peer": self._last_seq_by_peer,
            "seen_nonces": self._seen_nonce_order,
        ...
        try:
            ...
            logger.warning("[Pillar 30] Failed to save state to %s: %s", target, exc)
            return False
    ...
    def _load_state(self) -> bool:
        ...
        try:
            ...
            if not isinstance(data, dict):
                return False
            self._tx_seq = max(self._tx_seq, int(data.get("tx_seq", 0)))
            peer_seqs = data.get("last_seq_by_peer", {})
            if isinstance(peer_seqs, dict):
                for peer, seq in peer_seqs.items():
                    self._last_seq_by_peer[str(peer)] = max(
                        self._last_seq_by_peer.get(str(peer), 0),
                        int(seq)
                    )
            nonces = data.get("seen_nonces", [])
            if isinstance(nonces, list):
                for n in nonces:
                    n_str = str(n)
                    if n_str not in self._seen_nonce_set:
                        self._seen_nonce_set.add(n_str)
                        self._seen_nonce_order.append(n_str)
                if len(self._seen_nonce_order) > self.max_nonce_history:
                    excess = len(self._seen_nonce_order) - self.max_nonce_history
                    for _ in range(excess):
                        dropped = self._seen_nonce_order.pop(0)
                        self._seen_nonce_set.discard(dropped)
            self._handshake_accepts = int(data.get("handshake_accepts", self._handshake_accepts))
            self._handshake_rejects = int(data.get("handshake_rejects", self._handshake_rejects))
            self._sync_accepts = int(data.get("sync_accepts", self._sync_accepts))
            self._sync_rejects = int(data.get("sync_rejects", self._sync_rejects))
            return True
        except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
            logger.warning("[Pillar 30] Failed to load state from %s: %s", self.storage_path, exc)
            return False
    ...
    def status(self) -> Dict[str, Any]:
        return {
            ...
            "last_seq_by_peer": dict(self._last_seq_by_peer),
            "handshake_accepts": self._handshake_accepts,
            "handshake_rejects": self._handshake_rejects,
            "sync_accepts": self._sync_accepts,
            "sync_rejects": self._sync_rejects,
