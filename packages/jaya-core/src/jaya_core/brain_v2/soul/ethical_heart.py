...
_POLICY_EFFECTS_BY_CODE = {value: key for key, value in _POLICY_EFFECT_CODES.items()}
_POLICY_RISK_CODES = {risk: index for index, risk in enumerate(PolicyRisk)}
...
class PolicyFailureCode(str, Enum):
    ...
    APPROVAL_TRUST_UNAVAILABLE = "APPROVAL_TRUST_UNAVAILABLE"
...
class EthicalHeart:
    ...
    def __init__(
        ...
        *,
        ...
        approval_key_resolver: Callable[[str], bytes | str | None] | None = None,
        attestation_signer: AttestationSigner | None = None,
        attestation_verifier: AttestationVerifier | None = None,
        clock: Callable[[], datetime] = _utc_now,
        policy_gate: TrustedPolicyGate | None = None,
    ) -> None:
        active_policy = policy or default_core_policy("UNENROLLED")
        self._clock = clock
        self._policy = active_policy
        self._policy_gate = policy_gate or get_trusted_policy_gate()
        self._trusted_rules = tuple(
            TrustedPolicyRule(
                effect=_POLICY_EFFECT_CODES[rule.effect],
                risk_mask=sum(1 << _POLICY_RISK_CODES[risk] for risk in rule.risk_classes),
        ...
        if approval_public_keys and approval_key_resolver is not None:
            raise PolicyError(
                PolicyFailureCode.INVALID_INPUT,
                "static approval keys and dynamic resolver are mutually exclusive",
            )
    ...
    def evaluate(
        ...
        try:
            trusted_decision = self._policy_gate.evaluate(
                capability_id=request.capability_id,
                risk_code=_POLICY_RISK_CODES[request.risk_class],
                rules=self._trusted_rules,
                default_effect=_POLICY_EFFECT_CODES[self._policy.default_effect],
            )
            effect = _POLICY_EFFECTS_BY_CODE[trusted_decision.effect]
            rule = (
                self._policy.rules[trusted_decision.rule_index]
                if trusted_decision.rule_index is not None
                else None
            )
        except (KeyError, IndexError, NativeProviderError) as exc:
            raise PolicyError(
                PolicyFailureCode.POLICY_UNAVAILABLE,
                "trusted policy evaluator is unavailable or returned an invalid decision",
            ) from exc
    ...
    def _verify_and_consume_approval(
        ...
        public_key = self._resolve_approval_key(approval.approver_id)
    ...
    def _resolve_approval_key(self, approver_id: str) -> Ed25519PublicKey | None:
        if self._approval_key_resolver is None:
            return self._approval_keys.get(approver_id)
        try:
            material = self._approval_key_resolver(approver_id)
            if material is None:
                return None
            return self._load_approval_keys({approver_id: material})[approver_id]
        except PolicyError:
            raise
        except Exception as exc:
            raise PolicyError(
                PolicyFailureCode.APPROVAL_TRUST_UNAVAILABLE,
                "dynamic approval trust registry is unavailable",
            ) from exc
    ...
    def status(self) -> dict[str, object]:
        return {
            ...
            "approval_trust_source": (
                "dynamic_resolver"
                if self._approval_key_resolver is not None
                else "static_mapping"
            ),
            "signed_receipts": self._attestation_signer is not None,
            "policy_evaluator": self._policy_gate.profile(),
