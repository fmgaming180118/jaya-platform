"""Legacy in-process patch experiment.

STATUS: PROTOTYPE
BELUM PRODUCTION
BATASAN:
- tidak memiliki signed manifest, approval binding, canary, atau ledger;
- sengaja tidak dihubungkan ke IronEngine production;
- jalur produksi P24 adalah EvolutionInstaller/PromotionOrchestrator.
    ...
    def patch(
        ...
        if self._ethical_heart:
            try:
                ...
                payload_hash = hashlib.sha256(new_code.encode("utf-8")).hexdigest()
                req = PolicyRequest(
                    request_id=f"req-morphic-{int(time.time() * 1000)}",
                    actor_brain_id=self._ethical_heart.policy.brain_id,
                    node_id="local-node",
                    capability_id="core.logic.evaluate",
                    risk_class=PolicyRisk.READ_ONLY,
                    permissions=(),
                    payload_sha256=payload_hash,
                    purpose="morphic.patch",
                )
                decision = self._ethical_heart.evaluate(req)
                if decision.effect != PolicyEffect.ALLOW:
                    return self._reject(f"EthicalHeart denied patch: {decision.reason_code}")
            except Exception as exc:
                return self._reject(f"EthicalHeart evaluation failed: {exc}")
