"""Legacy migration experiment retained for compatibility research.

STATUS: PROTOTYPE
BELUM PRODUCTION
BATASAN:
- belum memakai owner-approval receipt dan migration ledger;
- belum menghasilkan format current `.jaya` secara one-way;
- tidak dihubungkan ke IronEngine production.
...
class LegacyProtocol:
    ...
    def __init__(self, jay_path: str, dna_secret: bytes):
        if not isinstance(dna_secret, bytes) or not dna_secret:
            raise ValueError("dna_secret must be injected as non-empty bytes")
    ...
    def _load_manifest(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("[Legacy] failed to load manifest from %s: %s", self._manifest_path, exc)
            return {}

    def _save_manifest(self) -> bool:
        try:
            target = Path(self._manifest_path)
            ...
            logger.error("[Legacy] cannot write manifest to %s: %s", self._manifest_path, exc)
            return False
