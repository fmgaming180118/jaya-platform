...
from jaya_core.model.native_architecture import JayaLibrarianNativeV0, HAS_TORCH
from jaya_core.paths import core_data_dir, core_models_dir
...
def train_librarian(
    ...
    resolved_dataset = (
        ...
        else core_data_dir() / "librarian_dataset.jsonl"
    )
    resolved_output = (
        Path(output_dir)
        if output_dir is not None
        else core_models_dir() / "jaya-core-v0"
    )
    dataset = LibrarianDataset(resolved_dataset, tokenizer, max_length=512)
    ...
    logger.info(f"Saving JAYA Core Librarian V0 to {resolved_output}")
    os.makedirs(resolved_output, exist_ok=True)
    
    # Save model config
    config = model.config
    with open(os.path.join(resolved_output, "model_config.json"), "w", encoding="utf-8") as f:
        json.dump(config, f, indent=2)
        
    # Save state dict
    weights_path = os.path.join(resolved_output, "model.safetensors")
    ...
    with open(os.path.join(resolved_output, "training_manifest.json"), "w", encoding="utf-8") as f:
