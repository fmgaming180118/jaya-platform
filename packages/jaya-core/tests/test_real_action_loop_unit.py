...
from jaya_core.security.approval_authority import ApprovalAuthority, ApprovalReceipt

from jaya_core.ai_connectors.cognitive_agent_bridge import CognitiveAgentBridge
from jaya_core.cognitive.contracts import RiskClass
from jaya_core.ai_connectors.cognitive_agent_bridge import DEFAULT_PROCESS_PROFILES
...
from jaya_core.reasoning.symbolic_reasoner import SymbolicReasoner, ResourceProfile, create_symbolic_reasoner
...
def test_approval_receipt_creation_and_verification():
    ...
    from jaya_core.security.approval_authority import ApprovalAuthority
    authority = ApprovalAuthority()
...
def test_cognitive_agent_bridge_real_file_write_and_read():
    ...
    # Real fs.write execution with signed ApprovalReceipt
    import json
    import os
    ...
    abs_path = str((Path.cwd() / test_file).resolve())
    ...
    try:
        ...
        # Real fs.read execution (no approval needed for read)
...
def test_p0_unauthorized_consent_rejected():
    ...
    assert "MISSING_APPROVAL_RECEIPT" in res["reason"] or "USER_CONSENT_REQUIRED" in res["reason"]
...
def test_iron_engine_cognitive_reason_and_act_structured_execution():
    ...
    step1 = MockActionStep("fs.write", {"path": "temp_engine_step.txt", "content": "engine test"}, required_capability="fs.write", risk_class=RiskClass.REVERSIBLE, approval_required=True, step_id="step-1", title="Write file")
    step2 = MockActionStep("fs.read", {"path": "temp_engine_step.txt"}, required_capability="fs.read", risk_class=RiskClass.READ_ONLY, approval_required=False, step_id="step-2", title="Read file")
