...
def test_homeostasis_no_alert_on_healthy_memory():
    ...
    from jaya_core.brain_v2.extensions.twin.experiment_memory import ExperimentMemory
    from jaya_core.brain_v2.extensions.twin.task_planner import TaskPlanner
...
def test_spontaneity_no_spark_when_busy():
    from jaya_core.brain_v2.organism.spontaneity import EntropySpark
    from jaya_core.brain_v2.extensions.twin.task_planner import TaskPlanner, Task, Priority
...
def test_legacy_spontaneity_never_fires_without_explicit_authorization():
    from jaya_core.brain_v2.organism.spontaneity import EntropySpark
    from jaya_core.brain_v2.extensions.twin.task_planner import TaskPlanner
    ...
    class FakeTwin:
        planner = TaskPlanner()
    twin = FakeTwin()
    # Idle time is no longer implicit authorization.
    spark.tick(twin)
    spark.tick(twin)
    assert len(twin.planner) == 0
    assert spark.status()["status"] == "EXPLICIT_AUTHORIZATION_REQUIRED"
    print("[OK] Spontaneity: idle trigger blocked without authorization")
...
def test_temporal_weight_decay():
    from jaya_core.brain_v2.engine.temporal_weights import TemporalWeighter
    from jaya_core.brain_v2.extensions.twin.experiment_memory import ExperimentRecord
...
def test_best_recent_reranks():
    ...
    mem._cache.append(ExperimentRecord(  # type: ignore[attr-defined]
...
def test_lingua_encode_action():
    from jaya_core.brain_v2.soul.lingua_logica import encode, decode
...
def test_lingua_encode_arithmetic():
    from jaya_core.brain_v2.soul.lingua_logica import encode, evaluate
...
def test_resource_monitor_readings():
    from jaya_core.brain_v2.organism.resource_monitor import get_readings
...
if __name__ == "__main__":
    tests = [
        ...
        test_legacy_spontaneity_never_fires_without_explicit_authorization,
