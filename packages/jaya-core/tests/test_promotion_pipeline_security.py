...
from jaya_core.brain_v2.engine._evolution_manifest_security import (  # noqa: E402
    signature_payload,
)
from jaya_core.brain_v2.engine.canary import (  # noqa: E402
    CanaryError,
    CanaryRegistry,
    GenericCanary,
    QLoRAAdapterCanary,
...
from jaya_core.brain_v2.engine.evolution_gate import (  # noqa: E402
    EvolutionCandidate,
    EvolutionGate,
)
...
def _setup_promotion_fixtures(
    tmp_path: Path,
    *,
    target_pre_content: bytes | None = None,
    canary_func: Any = None,
    target_rel_path: str = "models/candidate.safetensors",
) -> dict[str, Any]:
    now = time.time()
    ...
    target_full = data_root / target_rel_path
    if target_pre_content is not None:
        target_full.parent.mkdir(parents=True, exist_ok=True)
        target_full.write_bytes(target_pre_content)
        expected_restore = digest_bytes(target_pre_content)
    else:
        expected_restore = ABSENT_RESTORE_DIGEST
    ...
    manifest = build_manifest_v1(
        ...
        registry_key="candidate-safe",
        canary_check_id="qlora-adapter-inference-v1",
        expected_restore_digest=expected_restore,
        approver="human-reviewer",
        approval_id="approval-safe",
        approval_issued_at=now - 10,
        approval_expires_at=now + 3600,
    ...
    registry = CanaryRegistry()
    if canary_func is None:
        canary_func = lambda *_: {
            "passed": True,
            "mode": "real-inference",
            "probe_id": "trusted-probe",
            "model_loaded": True,
    ...
    registry.register(QLoRAAdapterCanary(canary_func))
...
def test_mid_install_canary_crash_recovers_cleanly_without_half_installed_state(
    tmp_path: Path,
) -> None:
    def crashing_canary(*_: Any) -> Mapping[str, Any]:
        raise RuntimeError("SIMULATED_CANARY_CRASH")

    # Case A: target file previously existed
    baseline_data = b"original baseline weights v1"
    ctx_a = _setup_promotion_fixtures(
        tmp_path / "case_a",
        target_pre_content=baseline_data,
        canary_func=crashing_canary,
    )
    with pytest.raises(EvolutionInstallError) as exc_info_a:
        ctx_a["orchestrator"].promote(
            candidate_document=ctx_a["candidate"].to_dict(),
            manifest=ctx_a["manifest"],
            test_report_path=ctx_a["test_path"],
            benchmark_report_path=ctx_a["benchmark_path"],
            artifact_source=ctx_a["artifact"],
            expected_commit=COMMIT,
        )
    assert exc_info_a.value.code == "CANARY_FAILED"
    assert ctx_a["target_full"].read_bytes() == baseline_data
    assert not list(ctx_a["data_root"].glob("**/*.installing"))
    assert not list(ctx_a["data_root"].glob("**/*.tmp"))
    receipts_dir_a = ctx_a["data_root"] / ".evolution-receipts"
    if receipts_dir_a.exists():
        assert not list(receipts_dir_a.glob("*.json"))
    rollback_dir_a = ctx_a["data_root"] / ".evolution-rollback"
    if rollback_dir_a.exists():
        assert not list(rollback_dir_a.glob("**/*.previous"))
    reg_doc_a = ctx_a["orchestrator"].installer._storage.load_registry()
    assert ctx_a["manifest"]["manifest_id"] not in reg_doc_a["installations"]
    assert not reg_doc_a["consumed_manifests"]

    # Case B: target file was absent before install
    ctx_b = _setup_promotion_fixtures(
        tmp_path / "case_b",
        target_pre_content=None,
        canary_func=crashing_canary,
    )
    with pytest.raises(EvolutionInstallError) as exc_info_b:
        ctx_b["orchestrator"].promote(
            candidate_document=ctx_b["candidate"].to_dict(),
            manifest=ctx_b["manifest"],
            test_report_path=ctx_b["test_path"],
            benchmark_report_path=ctx_b["benchmark_path"],
            artifact_source=ctx_b["artifact"],
            expected_commit=COMMIT,
        )
    assert exc_info_b.value.code == "CANARY_FAILED"
    assert not ctx_b["target_full"].exists()
    assert not list(ctx_b["data_root"].glob("**/*.installing"))
    assert not list(ctx_b["data_root"].glob("**/*.tmp"))
    reg_doc_b = ctx_b["orchestrator"].installer._storage.load_registry()
    assert not reg_doc_b["installations"]
    assert not reg_doc_b["consumed_manifests"]


def test_storage_full_io_error_aborts_cleanly_without_state_corruption(
    tmp_path: Path,
) -> None:
    baseline_data = b"original baseline weights safe"
    ctx = _setup_promotion_fixtures(tmp_path, target_pre_content=baseline_data)
    real_publish = ctx["orchestrator"].installer._storage.atomic_publish_copy

    def mock_publish(source: Path, target: Path, **kwargs: Any) -> None:
        if target == ctx["target_full"]:
            raise OSError(errno.ENOSPC, "No space left on device")
        return real_publish(source, target, **kwargs)

    with patch.object(
        ctx["orchestrator"].installer._storage,
        "atomic_publish_copy",
        side_effect=mock_publish,
    ):
        with pytest.raises(EvolutionInstallError) as exc_info:
            ctx["orchestrator"].promote(
                candidate_document=ctx["candidate"].to_dict(),
                manifest=ctx["manifest"],
                test_report_path=ctx["test_path"],
                benchmark_report_path=ctx["benchmark_path"],
                artifact_source=ctx["artifact"],
                expected_commit=COMMIT,
            )
        assert exc_info.value.code == "INSTALL_FAILED"

    assert ctx["target_full"].read_bytes() == baseline_data
    assert not list(ctx["data_root"].glob("**/*.installing"))
    assert not list(ctx["data_root"].glob("**/*.tmp"))
    reg_doc = ctx["orchestrator"].installer._storage.load_registry()
    assert not reg_doc["installations"]
    assert not reg_doc["consumed_manifests"]
