...
class _FakeHybrid:
    ...
    def status(self) -> Dict[str, Any]:
        return {
            ...
            "implementation_status": "test_double",
        }
    ...
    def _build_engine(self, node_id: str, secret: str, online: bool = True) -> IronEngine:
        with patch.dict(
            os.environ,
            {
                ...
                "JAYA_TWIN_SHARED_SECRET": secret,
                "JAYA_TWIN_STORAGE_PATH": str(self.temp_path / f"twin_{node_id}.json"),
                "JAYA_COLLECTIVE_PULSE_PATH": str(self.temp_path / f"pulse_{node_id}.json"),
            },
            clear=False,
        ):
