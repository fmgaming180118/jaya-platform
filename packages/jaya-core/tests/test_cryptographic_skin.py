...
def test_p13_wrong_secret_signature_and_unknown_field_fail_closed(
    ...
    assert wrong_key.value.code is CryptographicSkinFailureCode.AUDIT_CORRUPT
...
def test_p13_expiry_payload_bound_and_invalid_clock_fail_closed(
    ...
    current = [datetime(2026, 8, 14, 10, 0, tzinfo=UTC)]
    identity = _anchor(tmp_path / "identity")
    identity.enroll()
    skin = _skin(
        tmp_path / "core.db",
...
def test_p13_corrupt_audit_blocks_restart(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(database) as connection:
        connection.execute("UPDATE crypto_skin_audit SET payload_json = '{}' WHERE event_id = 1")
...
def test_p13_canonical_launcher_wires_validated_secret_and_identity(
    ...
    assert "invalid:JAYA_REQUIRE_CRYPTOGRAPHIC_SKIN_REQUIRES_IDENTITY" in dependency.value.issues
