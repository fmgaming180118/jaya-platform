...
from jaya_core.ai_connectors.connection_manager import ConnectionManager

class TestConnectionManager(unittest.TestCase):

    def setUp(self):
        # We'll mock the dependencies: LocalLLMAdapter, LANSyncClient, PublicAPIClient
        self.local_adapter_patch = patch('jaya_core.ai_connectors.connection_manager.LocalLLMAdapter')
        self.lan_client_patch = patch('jaya_core.ai_connectors.connection_manager.LANSyncClient')
        self.internet_client_patch = patch('jaya_core.ai_connectors.connection_manager.PublicAPIClient')
        ...
        self.value_scoring_patch = patch('jaya_core.ai_connectors.connection_manager.evaluate_intent_value')
        self.mock_value_scoring = self.value_scoring_patch.start()

        # Mock the filters
        self.filter_inbound_patch = patch('jaya_core.ai_connectors.connection_manager.filter_inbound')
        self.mock_filter_inbound = self.filter_inbound_patch.start()
        self.sanitize_outbound_patch = patch('jaya_core.ai_connectors.connection_manager.sanitize_outbound')
