...
from jaya_core.security.capsule import CapsuleKind
...
def test_p16_persistent_rotation_restart_and_revocation_contract(tmp_path) -> None:
    identity_root = tmp_path / "identity"
    anchor = DNAAnchor(
        identity_root,
        EncryptedFileKeyStore(
    ...
    lifecycle = PersistentQuantumAuthority(
        ...
        _policy(QuantumSuite.HYBRID_ED25519_ML_DSA_65),
        current_year=2026,
        provider_cls=_TestMLDSAProvider,
    )
    payload = b"persistent-p16-contract-artifact"
    first_key = lifecycle.active_key_id
    old_envelope = lifecycle.sign(payload)
    second_key = lifecycle.rotate()
    assert second_key != first_key
    ...
    # Re-open database after restart
    anchor = DNAAnchor(
        identity_root,
        EncryptedFileKeyStore(
            identity_root / "keystore", "p16-identity-" + ("i" * 40)
...
def test_p16_canonical_launcher_fails_closed_when_provider_unavailable(tmp_path) -> None:
    if OQSMLDSA65Provider().available():
        pytest.skip("liboqs is available; test is for unavailable provider behavior")
    data_root = tmp_path / "data"
    data_root.mkdir()
    identity_root = data_root / "identity"
    identity_secret = "p16-failclosed-identity-" + ("i" * 40)
    anchor = DNAAnchor(
        identity_root,
        EncryptedFileKeyStore(identity_root / "keystore", identity_secret),
    )
    ...
    assert exc_info.value.code is QuantumFailureCode.PROVIDER_UNAVAILABLE
