...
from jaya_core.brain_v2.engine.jaya_ir_translator import (
    logic_expr_to_ir,
    logic_request_to_ir,
    text_to_ir,
)
...
from jaya_core.operations.snapshot import (
    CoreSnapshotManager,
    SnapshotError,
    SnapshotFailureCode,
)
from jaya_core.reasoning import pure_logic
...
from jaya_core.resources.modes import ExecutionMode, ExecutionModeController
...
def test_p02_slow_hardware_probe_is_cached_with_age(
    ...
    monkeypatch.setattr("jaya_core.resources.profiler.shutil.which", lambda _: "nvidia-smi")
    monkeypatch.setattr("jaya_core.resources.profiler.subprocess.run", run_probe)
    profiler = ResourceProfiler(
