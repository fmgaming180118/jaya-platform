...
"""
    ...
    def create_markdown(self, pdfplumber_data: Dict, images: List, 
        ...
        md_lines.append(f"- **Mahasiswa**: {metadata.get('student_name', 'Tidak diketahui')}")
        md_lines.append(f"- **NIM**: {metadata.get('nim', 'Tidak diketahui')}")
        md_lines.append(f"- **Program Studi**: {metadata.get('program_studi', 'Tidak diketahui')}")
        md_lines.append(f"- **Tahun**: {metadata.get('year', 'Tidak diketahui')}")
        md_lines.append(f"- **Dosen Pembimbing**: {metadata.get('pembimbing', 'Tidak diketahui')}")
        md_lines.append("")
...
def main():
    # Input PDF
    pdf_path = Path(r"D:\Kampus\CRMxInquiryImport\Tugas Akhir - FACHRI FAHIRA PRANAJAYA.pdf")
    
    # Output directory
    output_dir = Path(__file__).parent.parent / "data" / "processed_ta"
