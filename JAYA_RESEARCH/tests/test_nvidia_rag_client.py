...
from research.enhanced_rag import NVIDIAEmbeddings, _normalize_vectors
...
class TestNVIDIARAGClient(unittest.TestCase):
    def setUp(self):
        ...
        with patch("research.nvidia_rag_client.NVIDIAEmbeddings", return_value=self.mock_embedder):
    ...
    def test_workspace_filter_is_respected(self):
        other = NVIDIARAGClient(vector_store_path=self.tmp, workspace_id="other_ws", use_embeddings=True)
        other.embedder = self.mock_embedder
