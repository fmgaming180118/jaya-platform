...
class UMLGenerator:
    ...
    def generate_diagram(self, spec: str, diagram_type: str) -> Tuple[str, str]:
        ...
        for attempt in range(max_attempts):
            if last_error:
                correction_prompt = (
                    f"{prompt}\n\n"
                    f"⚠️ PERINGATAN: Percobaan sebelumnya GAGAL karena kesalahan berikut:\n"
                    f"{last_error}\n\n"
                    f"Perbaiki kode PlantUML tersebut agar memenuhi semua aturan pedoman UML!"
                )
                print(f"[UML] Attempt {attempt+1}: Requesting correction from Teacher...")
                code = self.agent.ask(
                    correction_prompt, 
                    system_instruction="You are a strict UML designer. Output ONLY valid PlantUML code enclosed in @startuml and @enduml."
                )
            else:
                print(f"[UML] Attempt {attempt+1}: Generating {diagram_type} diagram...")
                code = self.agent.ask(
                    prompt, 
                    system_instruction="You are a strict UML designer. Output ONLY valid PlantUML code enclosed in @startuml and @enduml."
                )
            ...
            try:
                ...
                print(f"[UML] ✅ Diagram successfully validated on attempt {attempt+1}.")
        ...
        raise UMLValidationError(f"Gagal menghasilkan diagram UML yang valid setelah {max_attempts} percobaan. Error terakhir: {last_error}")
    ...
    def validate_diagram(self, code: str, diagram_type: str):
        ...
        if "@startuml" not in code or "@enduml" not in code:
            raise UMLValidationError("Diagram harus memiliki penanda @startuml dan @enduml.")
    ...
    def _validate_usecase(self, code: str):
        ...
        1. Actors cannot associate/interact with other actors directly (Actor1 -- Actor2 or Actor1 -> Actor2 is invalid).
        2. Use Cases must have at least one actor connection (either direct or indirect via extend/include).
        ...
        for line in lines:
            ...
            act_match = re.search(r'actor\s+("([^"]+)"|[\w\-]+)(?:\s+as\s+(\w+))?', line)
            if act_match:
            ...
            uc_match = re.search(r'usecase\s+("([^"]+)"|[\w\-]+)(?:\s+as\s+(\w+))?', line)
            if uc_match:
        ...
        for line in lines:
            if "--" in line or "->" in line or "<-" in line:
                # Exclude lines that contain keyword stereotypes like <<include>> or <<extend>>
                if "<<include>>" in line or "<<extend>>" in line:
                    continue
                # Split relation
                parts = re.split(r'--+|->+|<-+', line)
                ...
                if len(parts) >= 2:
                    ...
                    if p1 in actors and p2 in actors:
                        raise UMLValidationError(f"Pelanggaran Pedoman UML: Aktor '{p1}' tidak boleh terhubung langsung ke Aktor '{p2}'.")

        # 3. Check every use case has an actor connection (directly or via stereotypic include/extend)
        ...
        unconnected_ucs = usecases - visited
        if unconnected_ucs:
            raise UMLValidationError(f"Pelanggaran Pedoman UML: Use case berikut tidak terhubung ke Aktor mana pun: {', '.join(unconnected_ucs)}")

    def _validate_class(self, code: str):
        """
        Validates Class diagrams:
        1. Ensures relationship syntax uses standard arrow heads (`<|--`, *--, o--, -->).
        ...
        for line in lines:
            for arr in invalid_arrows:
                if arr in line and not line.startswith("'"):
                    raise UMLValidationError(f"Pelanggaran Pedoman UML: Simbol panah relasi '{arr}' tidak sesuai standar UML (Gunakan <|--, *--, o--, atau -->).")
        ...
        for line in lines:
            ...
            if any(k in stripped for k in ["class ", "interface ", "enum "]) or any(stripped.startswith(k) for k in ["class", "interface", "enum"]):
                # Ensure it's not a relation line or inline definition
                if "{" in stripped:
                    inside_class_block = True
                    continue
                elif ":" not in stripped and not any(sym in stripped for sym in ["--", "..", "->", "<-"]):
            ...
            if stripped.startswith("'") or stripped.startswith("/'") or stripped.startswith("@") or stripped.startswith("skinparam"):
                continue
            ...
            if ":" in stripped:
                if inside_class_block:
                    ...
                    if not any(stripped.startswith(x) for x in ["+", "-", "#", "~"]):
                        raise UMLValidationError(f"Pelanggaran Pedoman UML: Atribut/Metode '{stripped}' di dalam kelas harus dideklarasikan dengan visibilitas (+, -, #, ~).")
                else:
                    # Outside class block, check if it's an inline definition: ClassName : attribute
                    ...
                    if match:
                        ...
                        if not any(member_def.startswith(x) for x in ["+", "-", "#", "~"]):
                            raise UMLValidationError(f"Pelanggaran Pedoman UML: Atribut/Metode '{stripped}' harus dideklarasikan dengan visibilitas (+, -, #, ~).")

    def _validate_sequence(self, code: str):
        """
        Validates Sequence diagrams:
        1. Ensures activation/deactivation balance matches (every activate has a deactivate).
        """
        lines = code.splitlines()
        activations = {}
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if act_match:
                ...
                if activations.get(name, 0) <= 0:
                    raise UMLValidationError(f"Pelanggaran Pedoman UML: Lifeline '{name}' dideaktivasi pada baris {line_num} tanpa diaktivasi terlebih dahulu.")
                activations[name] -= 1
                
        # Check leftover activations
        for name, count in activations.items():
            if count > 0:
                raise UMLValidationError(f"Pelanggaran Pedoman UML: Lifeline '{name}' diaktivasi tetapi tidak dideaktivasi di akhir diagram.")
    ...
    def _validate_activity(self, code: str):
        ...
        if not has_start:
            raise UMLValidationError("Pelanggaran Pedoman UML: Activity diagram harus memiliki node mulai ('start').")
        if not has_stop:
            raise UMLValidationError("Pelanggaran Pedoman UML: Activity diagram harus memiliki node akhir ('stop' atau 'end').")
        ...
        for line in lines:
            if re.match(r'^if\s*\(', line):
                ...
                if_count -= 1
                if if_count < 0:
                    raise UMLValidationError("Pelanggaran Pedoman UML: Penutup 'endif' ditemukan tanpa pembuka 'if'.")
                    
        if if_count > 0:
            raise UMLValidationError(f"Pelanggaran Pedoman UML: Terdapat {if_count} blok 'if' yang tidak ditutup dengan 'endif'.")
    ...
    def get_render_url(self, code: str) -> str:
        ...
        return f"http://www.plantuml.com/plantuml/svg/~1{encoded}"
    ...
    def _build_prompt(self, spec: str, diagram_type: str) -> str:
        ...
        rules = {
            "usecase": (
                ...
                "- ATURAN MUTLAK: Dilarang keras menghubungkan aktor langsung ke aktor lain!\n"
                "- Setiap Use Case wajib terhubung langsung atau tidak langsung ke aktor."
            ),
            "class": (
                "- Definisikan atribut dan metode di dalam kelas\n"
                "- Wajib mencantumkan visibilitas (+ untuk public, - untuk private, # untuk protected)\n"
                "- Relasi menggunakan panah standar: pewarisan (<|--), komposisi (*--), agregasi (o--)."
            ),
            "sequence": (
                "- Cantumkan participant / lifeline\n"
                "- Aktivasi/deaktivasi lifeline wajib balance menggunakan 'activate lifeline' dan 'deactivate lifeline'."
            ),
            "activity": (
                "- Gunakan sintaks baru PlantUML activity (@startuml, start, :Activity;, stop, @enduml)\n"
                "- Wajib ditutup dengan 'stop' atau 'end'\n"
                "- Blok percabangan wajib diakhiri dengan 'endif'."
            )
        }
        
        prompt = f"""Tulis kode PlantUML untuk diagram {diagram_type.upper()} berdasarkan spesifikasi berikut:
...
if __name__ == "__main__":
    ...
    try:
        ...
        code, url = gen.generate_diagram("Aktor Mahasiswa login dan melihat nilai, Dosen login dan mengisi nilai.", "usecase")
        print("Code:\n", code)
        print("URL:\n", url)
