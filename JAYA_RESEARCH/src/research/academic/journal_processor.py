...
from src.research.academic.indonesia_sources import CrossrefIndonesiaClient, GarudaClient
from src.research.academic.citation_graph import get_citation_graph
...
class PaperCache:
    ...
    def put(self, paper_meta: Dict[str, Any], insight: str, local_path: str,
        ...
        self._index[self._key(title)] = {
            ...
            "pdf_link": paper_meta.get("pdf_link") or paper_meta.get("landing_page_url") or "",
    ...
    def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        results.sort(key=lambda x: (x["_cache_score"], x.get("rank_score", 0)), reverse=True)
...
class JournalProcessor:
    ...
    def process_query(self, query: str, max_papers: int = 1) -> Dict[str, Any]:
        ...
        if cached_hits:
            print(f"[JournalProcessor] Cache hit: {len(cached_hits)} paper(s) found, skipping download.")
            processed_data = []
            for entry in cached_hits:
                ...
                processed_data.append({
                    ...
                    "compact_summary": self._build_compact_summary(paper_meta, insight),
                    "local_path": entry.get("local_path", ""),
                    "from_cache": True,
                    "source_meta": {
                        "title": entry["title"],
        ...
        for paper in papers:
            ...
            if cached:
                print(f"[JournalProcessor] Paper cached: '{title}' — skipping download.")
    ...
    def _rank_papers(self, query: str, papers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def score(paper: Dict[str, Any]) -> float:
            ...
            return float(title_overlap + summary_overlap + source_overlap) + source_bonus + phrase_bonus + length_bonus + recency_bonus
    ...
    def _search_free_sources(self, query: str, max_papers: int = 1) -> List[Dict[str, Any]]:
        """Search all academic metadata sources (international + Indonesian) and deduplicate."""
        ...
        sources = [
            ...
            self.crossref_id.search_papers(query, max_results=max_papers),
            self.garuda.search_papers(query, max_results=max_papers),
        ]
...
Keep it concise (maximum 4 paragraphs). Focus on actionable insights relevant to the query.
"""

        return self.teacher.ask(
            prompt,
            system_instruction="You are a Research Scientist. Analyze the paper content and extract key insights relevant to the user's query. Be precise and technical."
        )
...
"""
        ...
        try:
            raw = self.teacher.ask(
                prompt,
                system_instruction="You are a reference extractor. Output raw JSON only."
            )
            raw = raw.strip().lstrip("```json").lstrip("```").rstrip("```").strip()
            refs = json.loads(raw)
            if isinstance(refs, list):
                print(f"[JournalProcessor] Extracted {len(refs)} references from '{paper_title}'.")
                return refs
        except Exception as e:
            print(f"[JournalProcessor] Reference extraction error: {e}")
        return []
