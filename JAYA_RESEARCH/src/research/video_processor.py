...
class SmartAudioProcessor:
    def split_by_silence(self, video_path: Path, output_dir: Path, min_duration: int = 240, max_duration: int = 300) -> List[Path]:
        """
    ...
    def _detect_silences(self, video_path: Path, db_threshold: int = -30, duration: float = 0.5) -> List[float]:
        """Use ffmpeg silencedetect filter to find silence timestamps"""
        cmd = [
            "ffmpeg", "-i", str(video_path),
            "-af", f"silencedetect=noise={db_threshold}dB:d={duration}",
            "-f", "null", "-"
        ]
        try:
    ...
    def _extract_segment(self, input_path: Path, output_path: Path, start: float, duration: float):
        # Precise extraction
        cmd = [
            "ffmpeg", "-ss", str(start), "-i", str(input_path),
            "-t", str(duration), "-c", "copy", "-y",  # Copy codec is fast/lossless
            str(output_path)
        ]
...
class AdaptiveVisualSampler:
    def sample_frames(self, video_path: Path, diff_threshold: float = 30.0, min_interval: float = 1.0, max_interval: float = 60.0) -> List[Any]:
        """
...
def retry_api(max_retries=5, initial_delay=1):
    def decorator(func):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "429" in str(e) or "500" in str(e) or "503" in str(e):
                        ...
                        print(f"[API RETRY] Attempt {attempt+1} failed. Retrying in {sleep_time:.2f}s... Error: {e}")
...
class VideoProcessor:
    ...
    def process_video_chunked(self, url: str) -> Dict[str, Any]:
        ...
        for i, chunk_path in enumerate(chunk_files):
            print(f"  > Processing Smart Chunk {i+1}/{len(chunk_files)}: {chunk_path.name}")
            ...
            visual_frames = adaptive_sampler.sample_frames(chunk_path, diff_threshold=30.0, min_interval=1.0, max_interval=60.0)
            ...
            if visual_frames:
                ...
                # Adjust timestamps for prompt context (relative -> relative to chunk start)
                ...
                # To simplify: We query VLM for the *chunk* summary, but we might want granular events.
                # For now, let's get a summary of the chunk's visuals.
                chunk_visual_desc = self._query_vlm(selected_frames, 
                    "Analyze these frames representing visual changes. Describe the key visual information, text, or interactions."
                )
            
            # Add to full visuals (mapped to chunk start time approx)
            full_visuals.append({
                "timestamp": base_timestamp, 
                "description": chunk_visual_desc
            })
    ...
    def _download_media(self, url: str):
        ...
        if os.path.exists(url):
            path = Path(url)
            return path, path, {"title": path.stem, "duration": "Unknown", "webpage_url": "Local File"}
    ...
    def _split_video_into_chunks(self, video_path: Path, output_dir: Path, chunk_duration: int) -> List[Path]:
    ...
    def _analyze_chunk_visuals(self, chunk_path: Path, chunk_index: int) -> str:
        ...
        if total_frames > 0:
            ...
            for i in range(0, total_frames, step):
                if len(frames_to_encode) >= 5: break
        ...
        prompt = f"Analyze these 5 frames from a video segment (Minute {chunk_index*5} to {(chunk_index+1)*5}). Describe the key events, people, and details shown."
    ...
    def _analyze_visuals(self, video_path: Path) -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            future_to_timestamp = {
                executor.submit(self._query_vlm, b64, "Describe the scene, people, and any text visible in this frame."): ts 
            ...
            for future in concurrent.futures.as_completed(future_to_timestamp):
                ...
                try:
                    ...
                    print(f"[VIDEO] Frame at {ts:.1f}s failed: {exc}")
    ...
    def _transcribe_audio(self, audio_path: Path) -> str:
        ...
        try:
            import whisper
            model = whisper.load_model("base") # Use 'base' or 'small' for speed, 'medium' for accuracy
    ...
    @retry_api(max_retries=5, initial_delay=2)
    def _query_vlm(self, base64_frames: List[Dict[str, Any]], prompt: str) -> str:
        ...
        timestamps = ", ".join([f"{f['timestamp']:.1f}s" for f in base64_frames])
