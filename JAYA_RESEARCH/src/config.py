...
class ResearchSettings:
    ...
    def load(
        ...
        *,
        ...
        base = Path(base_dir or Path(__file__).resolve().parent.parent).resolve()
        root = base.parent
        research_path = Path(
            config_path or base / "configs" / "research_config.yaml"
        ).resolve()
        research = _yaml(research_path, required=config_path is not None)
        if nim_config_path is None:
            root_nim = root / "nvidia_nim_config.yaml"
            nim_path = root_nim if root_nim.exists() else base / "nvidia_nim_config.yaml"
        else:
            nim_path = Path(nim_config_path)
        nim = _yaml(nim_path.resolve(), required=nim_config_path is not None)
