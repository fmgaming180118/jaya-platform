...
def main() -> int:
    ...
    privacy = SovereignPrivacy(database, secret, consent_public_keys={"owner-demo-key": public_key})
    ...
    allowed = privacy.evaluate(PrivacyUseRequest(**request_fields, consent_id=grant.consent_id))
    exported = privacy.export_owner(owner)
    deletion = privacy.delete_owner(owner)
    audit_valid = privacy.audit_chain_valid()
    privacy_scope_provider = privacy.status()["privacy_scope_provider"]
    privacy.close()
    ...
    result = {
        ...
        "privacy_scope_provider": privacy_scope_provider,
