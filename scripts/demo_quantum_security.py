...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-test-contract",
        action="store_true",
        help="Use test-contract provider if liboqs is unavailable on this host",
    )
    ...
    prod_provider = OQSMLDSA65Provider()
    if not prod_provider.available() and not args.allow_test_contract:
        report = {
            "status": "BLOCKED_EXTERNAL",
            "provider": prod_provider.provider_id,
            "algorithm": prod_provider.algorithm,
            "ml_dsa_ready": False,
            "reason": "maintained liboqs ML-DSA-65 provider is unavailable on this host",
        }
        print(json.dumps(report, ensure_ascii=False, indent=2))
        return 2

    provider_cls = OQSMLDSA65Provider if prod_provider.available() else _TestMLDSAProvider
    ...
    quantum = PersistentQuantumAuthority(
        database, skin, _policy(), current_year=2026, provider_cls=provider_cls
    )
    ...
    report = {
        "status": "INTEGRATED_LOCAL" if prod_provider.available() else "TEST_CONTRACT_VERIFIED",
        "provider": status["provider"],
        "versions": status["versions"],
        "ml_dsa_ready": status["ready"] and prod_provider.available(),
    ...
    if prod_provider.available():
        required = required + (report["ml_dsa_ready"],)
